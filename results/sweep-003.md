...
**Strategy note — Combinatorial discipline**: The 24 items below are NOT a flat list of independent fixes. They form a gated experiment tree. Three axes (graph size, LC ratio, info scale) interact, and crossing all of them with outlier_rate(5) × structure(2) × seed(3+) produces hundreds of training runs. The project's own `implementation_details.md` already applies this discipline (not crossing the K-sweep with the outlier-rate sweep). The same logic applies here:

- **Phase A**: Fix outlier_rate/structure at representative points (50pct_clustered as best all-around per sweep-003, 50pct_random as the strongest generalization performer on Intel, plus 90pct_random as the conservative extreme), and sweep graph-size × LC-ratio × info-scale under BCE only (cheap, no solver in training loop). This answers: *how much of the generalization gap is pure data-distribution mismatch?*
- **Phase B**: Take whichever domain-profile from Phase A most closes the gap, lock it as the new default, and run the full outlier_rate × outlier_structure × seed grid on top — with both BCE and trajectory-loss. This answers: *what does trajectory loss buy over BCE on the improved distribution?*
...
| 4 | **Vary graph size in training sweep** | Split by loss function. **BCE arm**: no constraint — PyG's disjoint-union batching handles mixed graph sizes natively. Can train on a continuous range, not just discrete buckets. **Trajectory-loss arm**: `implementation_details.md`'s batching section requires matching size/topology within a batch, and K × graph_size memory compounds with full-unrolling. Cap trajectory-loss at 100–300 poses. Treat "does trajectory loss also need to scale to 1000+ node graphs" as gated on gradient checkpointing being built (Future Work). Initial BCE sweep grid: `data.num_poses ∈ {100, 500, 1000, 3500}` — 4 discrete sizes spanning the training-to-benchmark range (100 to city10000). |
| 5 | **Vary info matrix scale in training** | Sweep a **scalar multiplier** on fixed diagonal shapes, not per-edge distributions. Follows precedent from Khosoussi & Shames (heteroscedastic design on M3500: odometry info held fixed, LC info shape fixed but scaled by a swept multiplier). Apply α ∈ {0.1, 1, 10, 100} — log-spaced across 4 orders of magnitude — to the existing `diag(500,500,100)` / `diag(100,100,50)` shapes. Only scale the LC info matrix (odometry kept at fixed magnitude, matching the precedent). Real data spans 7 orders (CSAIL: 35–25,000,000), so 4 orders on first pass; widen if the model shows scale insensitivity. Fixes the "single point in a 10⁷× range" gap. |
| 6 | **Vary odometry:LC ratio in training** | Replace independent `num_loop_closures` sweep with **target ratios** computed from graph size. At fixed `num_poses`, ratio is set entirely by `num_loop_closures = num_poses / ratio` — sweeping both as free Cartesian axes multiplies redundant combinations. Use target ratios {1:1, 5:1, 15:1, 40:1} that bracket the real dataset range (city10000's 0.94:1 through MIT's 40:1). Don't try to hit each dataset's ratio exactly — that would look like fitting the training set to eval targets. The 15:1 ratio gives intermediate representativeness (between dense-grid Intel at 2.2:1 and corridor CSAIL at 8.2:1). |
...
| # | Action | Why |
|---|--------|-----|
| 11 | **Real-benchmark outlier-rate sweep** | Sweep `injection_outlier_rate=10,30,50,70` on each real benchmark. Currently injects a fixed 30% outlier rate. **LC-count scaling**: currently injects a flat 20 LCs regardless of graph size — this is 0.37% of M3500's 5,453 edges and 0.56% of Intel's 2,512 edges. LC count should scale with node count to hit a target ratio from the training distribution range (from Batch 1 #6). Pick a ratio the model was trained on and inject accordingly: e.g., at 15:1 ratio, Intel (1,728 nodes) gets ~115 injected LCs, M3500 (3,500 nodes) gets ~233. **Add-on-top convention**: injected LCs are added on top of the `.g2o` file's organic loop closures. Original edges get label=-1 sentinel and are excluded from edge-F1. Only injected edges participate in classifier evaluation. This is already implemented but undocumented — worth one line in `implementation_details.md`. |
| 12 | **Full information matrices in synthetic generator** | Add configurable non-zero off-diagonals to training graphs. Real datasets (Intel, MIT, CSAIL) have full 3×3 matrices with significant off-diagonal terms. **Gate**: only pursue this once the diagonal-scale sweep (Batch 1 #5) shows that scale alone explains meaningful variance. Sampling valid off-diagonal PSD matrices requires proper construction (Wishart sampling or random rotation of a diagonal matrix) to guarantee positive-semidefiniteness — not just "add noise to more entries." The Khosoussi & Shames precedent flags that estimating full covariance under sparse graph-structured measurements is harder than the point-cloud/PnP setting they extend from. |
...
### Batch 3 — LATER (higher effort, architecture + advanced features)
...
| 19 | **Increase outlier injection on real benchmarks** | Scale injected LC count with node count to hit target ratios from training (see Batch 2 #11). M3500's 20 injected LCs are 0.37% of 5,453 edges — the odometry chain dominates regardless of GNN weights. At 15:1 ratio, M3500 gets ~233 injected LCs which genuinely stresses outlier rejection. **Exclude CSAIL** — the convergence problem (§13B: all-zero initial poses from edge-only format, 8/10 models hit 100-iter cap) is orthogonal to injection scale and won't be fixed by this. CSAIL needs a different initialization strategy for edge-only files, which is a separate infra task. |
...
| 23 | **Residual-Guided Iterative Re-Weighting** | Core idea: instead of one-shot GNN prediction, run solve → compute per-edge residuals → feed residuals as new edge features → GNN re-predicts weights → repeat 2-3×. This gives the GNN the same adaptive feedback loop that GNC and DCS have built in mathematically. The GNN initially removes clearly-wrong edges (its strength: perfect precision), then sees which removed edges had small residuals (they actually fit the solution), and restores them in subsequent passes. **~30 lines** in `evaluate_one_graph()` for eval-only. **Requires retraining** with residuals as input features to be fully effective (~100 lines training changes). Directly addresses the core finding from this sweep: the GNN is confidently wrong about some edges because it has never seen optimization residuals. |
...
### Batch 4 — Phase 2: Online/Incremental + VSA/SSP (stretch goals)

| # | Action | Why |
|---|--------|-----|
| 25 | **Online / Incremental GNN Adaptation** | Adapt the GNN to operate in streaming SLAM: new poses and loop closures arrive incrementally. Key challenges: (a) k-hop neighborhood re-prediction (only re-score edges within the GNN's 3-layer message-passing radius when a new LC arrives), (b) solver warm-start from previous optimized poses via `gtsam.Values()`, (c) incremental graph builder to maintain the `PoseGraph` in memory as edges arrive. **~350 lines** total (graph builder + k-hop extraction + warm-start). Current architecture supports variable-size graphs natively, but edge classification is inherently a batch problem — k-hop partial re-prediction is the pragmatic compromise. |
| 26 | **Online Learning via Continual Adaptation** | Fine-tune the GNN on each new graph as it arrives, without catastrophic forgetting. Candidate approaches: (a) elastic weight consolidation (EWC) — penalize changes to important weights, (b) simple replay buffer of past synthetic graphs, (c) VSA/SSP algebraic updates (training-free, Phase 2's planned vehicle). **~200 lines**. Evaluate whether GNN-based online learning outperforms VSA's training-free algebraic adaptation — this comparison is the Phase 2 contribution story. |
| 27 | **VSA/SSP Baseline** (already planned, `architecture.md` §8) | Encode relative pose measurements as Spatial Semantic Pointers. Algebraic binding/unbinding scores loop-closure consistency — a consistent cycle of edges composes back to ~identity. Route deviation-from-identity as a training-cheap, structurally interpretable outlier signal. Integration point: standalone baseline through `evaluate.py` before any fusion into the GNN pipeline. |
