...
**Real-benchmark caveat (important, affects the Train/Val/Test protocol
below):** `gt_node_poses` is only genuinely available for the **simulated**
real benchmarks — M3500 (and its noisy variants) and Sphere2500 ship with a
true ground-truth trajectory, since they're generated by a known simulator
(Olson et al. 2006 / Carlone's SE-Sync suite), not collected from a real
sensor. **Intel and MIT/CSAIL are real robot logs** (raw wheel odometry +
laser range finder) with no independently-sourced dense ground truth — no
motion capture, no GPS. Any "ATE" reported on Intel/MIT in the literature is
relative to a reference optimized trajectory, not true ground truth. See
"Domain-Shift Characterization Metrics" below for how this splits the
eval-once protocol.
...
- **M3500 (+ variants) and Sphere2500** are simulator-generated and ship
  with a true `gt_node_poses`-equivalent trajectory. Both **edge-F1** and
  **true Umeyama-aligned ATE** are reportable here, and these are the
  primary quantitative generalization-gap numbers.
- **Intel and MIT/CSAIL** are real sensor logs with no independently
  sourced ground-truth trajectory. Edge-F1 is still reportable (see outlier
  injection below), but any "ATE" here is relative to a reference optimized
  trajectory, not true ground truth, and must be labeled as such wherever
  reported — never presented alongside M3500/Sphere2500 ATE numbers without
